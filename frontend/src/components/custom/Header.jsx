...
function Header({ loggedIn, setLoggedIn }) {
  ...
  return (
    <div className='shadow-sm flex flex-col justify-between sm:flex-row items-center px-10 py-3'>
      ...
      <button onClick={navHandler} className='border-none px-2 font-medium bg-gray-800 text-gray-100 sm:hidden'>Navbar</button>
      <div className={`flex flex-col sm:flex-row sm:h-auto opacity-100 items-center gap-3 transition-all duration-500 ease-in-out ${nav ? 'h-auto opacity-100' : 'h-0 opacity-0 overflow-hidden'
        }`}>
        <div>
          <NavLink to='/about'>
